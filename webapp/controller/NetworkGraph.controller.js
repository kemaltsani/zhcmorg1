    sap.ui.define(
        ...
        function (JSONModel, Controller, LayeredLayout, ForceBasedLayout, ActionButton, Node, Fragment) {
            "use strict";
            var GraphController = Controller.extend("zhcmorg1.controller.NetworkGraph");
    
            // var STARTING_PROFILE = "11000148";
            var STARTING_PROFILE = "";
    
            GraphController.prototype.onInit = function () {
                this.oComponent = this.getOwnerComponent();
                this._oModel = this.oComponent.getModel("graph");

//                this._oModel = new JSONModel(sap.ui.require.toUrl("model/graph.json"));
                ...
                this._oModel.read("/OrgSet('123')", {
                    async: false,
                    urlParameters:
                    {
                        $expand: "OrgToNodes,OrgToNodes/NodesToAttribute,OrgToLines"
                    },
                    success: function (oData, response) {
                        var oModel = new JSONModel({
                            OrgToNodes: oData.OrgToNodes.results,
                            OrgToLines: oData.OrgToLines.results
                        });
                        test.getView().setModel(oModel);  

                        STARTING_PROFILE = oData.OrgToNodes.results[0].id;
                        test._sTopSupervisor = STARTING_PROFILE;
                        test._mExplored = [test._sTopSupervisor];     
                        test._setFilter();                   
			    },
                    error: function (e) {
                        that._oBusy.close();
                        var lvErr = e.responseText + " in Entity " + lvPath;
                        console.log(lvErr);
                    }
                });
                ...
                this._graph.attachEvent("beforeLayouting", function (oEvent) {
                    ...
                    this._graph.getNodes().forEach(function (oNode) {
                        var oExpandButton, oDetailButton, oUpOneLevelButton, oEditButton,
                        ...
                        // add Edit link -> custom popover
                        oEditButton = new ActionButton({
                            title: "Edit",
                            icon: "sap-icon://edit",
                            press: function (oEvent) {
                                this._editDetail(oNode, oEvent.getParameter("buttonElement"));
                            }.bind(this)              
                        });
                        oNode.addActionButton(oEditButton);
            ...
            GraphController.prototype.suggest = function (oEvent) {
                var aSuggestionItems = [],
                    // aItems = this._oModel.getData().OrgToNodes,
                    aItems = this.getView().getModel().oData.OrgToNodes,
                    aFilteredItems = [],
            ...
            GraphController.prototype._openDetail = function (oNode, oButton) {
                ...
                if (!this._oQuickView) {
                    Fragment.load({
                        name: "zhcmorg1.view.TooltipFragment",
                        type: "XML"
...
//             GraphController.prototype._editDetail = function (oNode, oButton) {
//                 // var loRouter = sap.ui.core.UIComponent.getRouterFor(this);
//                 // loRouter.navTo("RouteNetworkGraphEdit");
// //                credit_request_no: window.encodeURIComponent("3")
               
//                 // this.getOwnerComponent().getTargets().display("RouteNetworkGraphEdit" , {
//                 //     idPath: oNode.getProperty("key")
//                 //   });

//                 // this.getOwnerComponent().getTargets().display("RouteNetworkGraphEdit");

//                 const oItem = oButton.getSource();
//                 const oRouter = this.getOwnerComponent().getRouter();
//                 oRouter.navTo("RouteNetworkGraphEdit", {
//                     invoicePath: window.encodeURIComponent(oItem.getBindingContext("graph").getPath().substr(1))
//                 });
//             };
